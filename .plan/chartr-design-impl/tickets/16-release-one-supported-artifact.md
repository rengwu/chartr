...
`./cmd/chartr`, `CGO_ENABLED=0`, `goos: [darwin, linux, windows] × goarch:
...
Windows, each bundling the `chartr` binary plus the README and ADR 0011; a
...
×2) with a `checksums.txt` and the Windows `.zip` containing `chartr.exe` (8.7

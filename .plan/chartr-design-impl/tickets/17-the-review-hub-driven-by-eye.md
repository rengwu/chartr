...
brief lived at .chartr/run/s1433d0d2444e/ (gitignored, per-session).
-->
...
- **The re-review shadow — a per-ticket pointer, not a session-tab scan.**
  ...
  (`.chartr/run/reviews/<slug>/<num>`, the session id, plain text)
  ...
  → fresh-pass, then restarts the chartr process entirely (fresh, empty
...
- **Trailer-identified work commits.** Every chartr lifecycle write (claim,
  release, promote, demote) now carries `Chartr-Write: true`; `workCommits`
  reads the full commit body (`%B`, not just `%s`) and `isChartrCommit` matches
...
- `TestReReviewSupersedesStaleAndSurvivesRestart` — reject → send-back → fresh
  pass, then a full chartr restart on the same data dir before asserting the
...
- **The browser pass itself did not run.** The Chrome extension was not connected
  ...
  instead: the actual binary (`go build ./cmd/chartr`, the freshly-built `web/
...
- **`resetAvailable` is correct but rarely true through the hub as built.** The
  ...
  loosening `tipOf` to tolerate interposed chartr commits and reverted it once I

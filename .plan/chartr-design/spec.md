...
15. As a teammate cloning a space, I want map kind declared in committed workspace config, so that my chartr and yours agree on which maps gate review without either of us re-classifying.
16. As an operator, I want the chartr to notice when `to-tickets` produces an implementation map and surface it for classification, so that graduation is observed rather than orchestrated.

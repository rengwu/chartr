...
- **Registering.** Point the chartr at a path — then what? Must it already be a git repository, and must it already have a `.plan/`? A repo with no maps is legal; ticket 07 charts one. What does the chartr write, and *where*: this is the first chartr-owned state that is not a space's own committed config, so it needs a home and a story for what happens when it is lost.
- **Discovery.** wayfinder-maps' picker already lists every map under a project's `.plan/`, and that code is liftable. The chartr needs it plus a layer above: many repos, each with many maps.
...
- **Removal.** De-registering a space that has live work, and whether the chartr ever touches the repository on the way out. It should probably touch nothing — but say so deliberately.
...
**Removal — forget, not destroy.** De-registering **touches nothing in the repository** — not `.plan/`, not the working tree, not git, not committed workspace config; it forgets the registry entry and its local pin/recency, nothing else. Re-register any time and the repo picks up exactly as it sits (commits in git, a dirty tree still dirty). With a **live session**, removal reclaims the **chartr-owned session process** but leaves every byte it wrote untouched — the clean split the whole design rests on: the chartr owns the *process*, the operator owns the *tree*. Trust-at-the-gate, said deliberately as the ticket asked.

...
One decision for review to weigh: **classification writes `.wayfinder-harness.toml` but does not `git commit` it.** The spec enumerates the harness's own commits as exactly claim / promotion / demotion (ADR 0008), none of which machinery exists yet; "committed config" is read here as the version-controlled *layer* (as ticket 02's bindings are — the operator commits them), and leaving the write in the working tree matches "operator owns the tree" and dirty-tree-is-a-badge. If ADR 0007's "recorded in harness-owned config committed to the space's repo" is meant as *the harness commits it*, this is the line to change — the write path is isolated in `handleClassify`, so adding a pathspec-limited commit later is local.

...
- **The last ticket.** The rule new routes must satisfy (06) is still fog; it now
  carries the constraints 03, 04 and 05 settled, so it decides what is left rather
  than re-deriving the credential. It also owns the ADR: one boundary, one numbered
  decision, with 04's bind rule as a clause rather than an ADR of its own.

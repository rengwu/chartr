...
- **The bound-address indicator, which 04 has now produced a need for.** A
  non-loopback instance should say so in the cockpit and not only in a startup line a
  weeks-old process scrolled away: a fact, not an alert, present only when the
  listener is wide. It rides the post-admission snapshot, so it discloses nothing to
  anyone not already admitted. Frontend work under CLAUDE.md and ADR 0012, chrome side
  of ADR 0010, monochrome — pairs with the denial page and needs its own ticket.

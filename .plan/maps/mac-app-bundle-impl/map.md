...
- **04 — ship it on a tag**: three files, no new job and no new target. The `shells` matrix job gains **one** step — `make dmg WEBVIEW_VERSION=${GITHUB_REF_NAME#v}`, guarded `matrix.goos == 'darwin' && steps.build_shell.outcome == 'success'`, deliberately **without an `id`** so nothing can later branch on its outcome — and the upload glob widens to `build/shell/* build/macapp/*.dmg build/macapp/*.dmg.sha256`. Naming the two files rather than the directory is what ticket 03's scratch-root deletion bought: `build/macapp/` still holds the staged bundle *directory*, which `gh release upload` would choke on. `continue-on-error` and `needs: release` are untouched, and packaging runs **inside** that job, which is the structural guarantee. **The failure mode was driven, not reasoned about**: with `hdiutil` sabotaged, `make dmg` exits 2 leaving no `.dmg` and no sidecar behind, the failing step fails the job, and the upload step's status-function-free `if:` gets an implicit `success()` ANDed in, so it is skipped entirely — nothing attaches, release untouched. **The deliberate consequence, raised not hidden: "nothing" includes the loose macOS shell**; buying it back would mean `if: always()`, exactly the softening the structure prevents. The happy path was replayed verbatim on a real Mac and resolves to exactly four assets (shell + sidecar, image + sidecar) — no directory, no stray `.DS_Store`, `checksums.txt` never named. The README gains a *Best-effort — the macOS app in a disk image* subsection carrying ticket 03's wording checked line-by-line against the `READ ME FIRST.txt` read back out of a freshly mounted image, plus the unsigned/un-notarized statement, Apple Silicon `arm64` named with Intel operators sent to the supported binary, and the diagnostic line `/Applications/chartr.app/Contents/MacOS/chartr`; the goreleaser footer gains one matching paragraph, and the workflow's header comment gains the image in its second bullet. No ADR work — 0016 already covers it. Unverifiable from here, and stated as such: the first real `v*` push. [ticket](tickets/04-ship-it-on-a-tag.md)

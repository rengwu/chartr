...
Capture the design system now that it is real, so every future session — human-driven or chartr-spawned — holds it without being re-taught. Four deliverables:
...
- **`prompts/implement.append.md`** — a short layered append (per the prompt library's override mechanism) so chartr-spawned *implementation* sessions that touch `web/` are told to follow `docs/design-system.md`. Keep it to the frontend rule; don't restate the whole doc.
...
- **`.chartr/prompts/implement.append.md`** — short layered append
  (per the prompt library's `<part>.append.md` mechanism) telling chartr-spawned
  ...
  *content* ships to everyone) is `.chartr/prompts/`, which is

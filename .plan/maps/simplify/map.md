...
**The previous design map is precedent, not scripture.** `.plan/chartr-design/` settled the architecture being cut here. Its standing preferences still bind: chartr is a **cockpit, not an autopilot**; the client is **hackable** — everything chartr injects or reasons with is visible on disk as plain files, never sealed in the binary; deterministic code owns what must always be true, agents exist only where judgment is the product.
...
- **Everything chartr injects becomes a `SKILL.md`.** Role prompts, the core "how to use chartr" prompt, and the tracker convention (map format, kinds, statuses, ticket flow) are repackaged as standard skill directories — hackable, open, and usable by agents outside chartr. Location, management, and injection mechanics are this map's ticket 02 to settle.
- **xterm.js stays.** The ghostty-web WASM terminal (xterm.js-compatible, libghostty-vt core) is banked as a revisit candidate if terminal UX still disappoints after the cut; server-side libghostty via cgo is rejected outright (kills the cgo-free supported artifact, drags in the Zig toolchain, pins an unstable API). If terminal bugs resurface, first suspect chartr's own raw-byte scrollback replay, not the renderer.
...
- **Redesigning the wayfinder method.** Same boundary as the design map: chartr drives wayfinder maps; the tracker-convention skill restates the format, it does not change it.

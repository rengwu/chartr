...
- **What survives of the skill-sync procedure and its provenance.** `SourceCommit`, the content hashing, `forked_from` and the stale-fork warning were all built for a vendored library that no longer exists — but the claim trailer still wants to record what actually ran. Which of that machinery is re-pointed at sources and which is deleted outright. *Anchored to [chartr-skills, and how it ships](tickets/05-chartr-skills-and-how-it-ships.md).*

...
- **Injection.** Today chartr composes one payload file and types a one-line opener. With skills, the options diverge: (a) keep composition — skills are sources chartr assembles into the payload as today; (b) materialise skill dirs into the agent CLI's *native* skills path for the session (per-adapter knowledge of where each CLI reads skills); (c) point the opener at skill files on disk and let the agent read them. Each trades determinism, agent-agnosticism, and openness differently. Pick one, and say what the context bundle (map body, ticket, blockers' answers) becomes — it is not a skill; does it stay a composed payload section?
- **Management and sync.** The skills are vendored from the upstream skills repo today. As SKILL.md dirs, does chartr keep vendoring (drift risk the old ticket accepted), point at a checkout, or stop owning content and ship only the convention? What does the hackable surface look like on disk after first run?
- **What chartr still guarantees.** Ticket 04's answer leaned on composition to *force* certain content (the old review prompt's Done-when + spec). With injection opened up, which guarantees survive in code, and which degrade to convention?

## Answer

**The prompt library becomes a skill library — standard `SKILL.md` directories on disk — but chartr keeps composing.** The format opens up; the injection path does not. Each role, the core, and the tracker convention ship as vendored skill directories the operator can read, edit, and reuse *outside* chartr; at spawn chartr still assembles core + role + a freshly-built context bundle into one gitignored payload and points the agent at it with a one-line opener, exactly as today (chartr-design ticket 04, retained). The openness win is the on-disk format and the standard's name/description contract; the control chartr keeps is deterministic assembly. Layers combine by **whole-skill shadowing** — most-specific skill wins outright, no per-file merge, no bespoke `replace`/`append`. This supersedes chartr-design ticket 04's prompt-library answer and **reaffirms ADR 0002** rather than amending it: chartr still injects prompts it composes itself and leans on no agent's skill mechanism.
...
- **`core`** — the "how to use chartr" skill: what a session is, the ground rules, commit conventions, *never push*, how work is recorded. Today's `core.md` body, now a `SKILL.md`. Injected first for every role, unchanged.
...
Rejected: **point at an external skills checkout** (chartr would depend on files it cannot version and cannot guarantee exist — rejected in chartr-design ticket 04 and still true); **stop owning content, ship only the convention** (a bare install would spawn sessions with no role wiring at all — the wired role prompts *are* chartr's value, and "open" does not mean "empty").

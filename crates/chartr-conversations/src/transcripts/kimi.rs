...
impl Reader {
    pub(super) fn read_kimi(
        ...
        let path = session_log(paths, native)?;
...
pub(super) fn session_log(paths: &ProviderPaths, native: &NativeSession) -> Result<PathBuf> {
    Ok(session_directory(paths, native)?.join("agents/main/wire.jsonl"))
}

pub(super) fn session_directory(paths: &ProviderPaths, native: &NativeSession) -> Result<PathBuf> {

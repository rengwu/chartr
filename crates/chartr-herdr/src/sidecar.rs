//! The herdr executable chartr ships, resolved by path and never through `PATH`.
//!
...
impl Sidecar {
    /// The herdr beside chartr's own executable, as the build script placed it.
    ///
    /// `PATH` is deliberately not consulted. A herdr the user installed for
    /// themselves is theirs; picking it up would make chartr's backend version
    /// depend on the machine.
    pub fn beside_current_exe() -> Result<Self> {
        let exe = std::env::current_exe().map_err(|err| {
            Error::Sidecar(format!("cannot locate chartr's own executable: {err}"))
        })?;
        let dir = exe
            .parent()

//! Places the vendored herdr beside the chartr binary.
//!
//! chartr resolves its backend by path — `<dir of the chartr executable>/herdr` —
//! and never through `PATH`, so the executable has to actually be there. This
//! copies it, and fails the build if it is not vendored, because a chartr that
...
fn workspace_root() -> PathBuf {
    // `CARGO_MANIFEST_DIR` is `crates/chartr`; the workspace is two above it.

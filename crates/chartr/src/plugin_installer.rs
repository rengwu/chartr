...
//!
//! Discovery remains in `chartr-plugin-host`: this module performs the
...
mod tests {
    ...
    fn a_local_source_cannot_contain_the_managed_staging_directory() {
        let temp = tempfile::tempdir().unwrap();
        write(
            temp.path().join("chartr-plugin.toml"),
    ...
    fn recorded_source_survives_activation_and_enable_cycles(cx: &mut gpui::TestAppContext) {
        ...
        let mut catalog = cx.update(|cx| chartr_plugin_host::load_all_where(&paths, |_| false, cx));
    ...
    fn upgrades_and_reenable_keep_live_code_until_the_next_startup(cx: &mut gpui::TestAppContext) {
        ...
        let live = paths.installed.join(id);
        package(&live, "old");
        let mut catalog = cx.update(|cx| chartr_plugin_host::load_all(&paths, cx));
        let source = temp.path().join("source");

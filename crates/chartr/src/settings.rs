...
const HEADER: &str = "\
# chartr user settings. Omitted fields use chartr's defaults.
...
const SIDEBAR_THEME_PALETTES: [SidebarThemePalette; 15] = [
    ...
    SidebarThemePalette::new(DEFAULT_DARK_THEME, 0x2e3540, 0x323a46, 0x384250, 0x3e495a),
    SidebarThemePalette::new(DEFAULT_LIGHT_THEME, 0xedf0f4, 0xe5eaf2, 0xd6deea, 0xc6d3e6),
];
...
fn chartr_dark(source: &Theme) -> Theme {
    ...
    dark.name = DEFAULT_DARK_THEME.into();
...
fn chartr_light(dark: &Theme) -> Theme {
    ...
    light.name = DEFAULT_LIGHT_THEME.into();
...
mod tests {
    ...
    fn chartr_dark_keeps_structural_borders_clear_of_its_surfaces(cx: &mut TestAppContext) {
        cx.update(|cx| {
            theme::init(theme::LoadThemes::JustBase, cx);
            init_themes(&ResolvedSettings::default(), cx);
            let theme = ThemeRegistry::global(cx).get(DEFAULT_DARK_THEME).unwrap();
            let colors = &theme.styles.colors;
    ...
    fn a_missing_file_resolves_to_shipped_defaults() {
        ...
        assert_eq!(store.resolved().fixed_theme, DEFAULT_DARK_THEME);

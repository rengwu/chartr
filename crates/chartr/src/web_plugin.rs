...
use chartr_plugin_host::FileBroker;
...
pub fn pane(
    ...
    context: chartr_plugin::InstanceContext,
    window: &mut Window,
    cx: &mut App,
) -> PluginView {
    let (view, webview) =
...
fn create_view(
    ...
    session: Option<SessionAccess>,
    on_focus: Option<FocusHandler>,
    context: Option<chartr_plugin::InstanceContext>,
    window: &mut Window,
    cx: &mut App,
...
fn handle_request(
    ...
    let result = match request.action {
        HostAction::ProjectRead { path } => broker
            .open_project(&path)
            .and_then(|file| read_file(file).map_err(chartr_plugin_host::BrokerError::Io))
...
mod tests {
    use super::*;
    use chartr_plugin::manifest::ProjectAccess;

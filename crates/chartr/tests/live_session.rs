...
//!
//!     cargo test -p chartr --test live_session -- --ignored --nocapture
...
fn sidecar() -> Sidecar {
    let herdr = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .canonicalize()
        .expect("build chartr first so herdr is vendored beside it");
...
fn a_created_session_has_a_native_direct_attach_target() {
    let live = Live::start();
    let workspace =
        live.client.open_workspace(&std::env::temp_dir(), Some("chartr-live")).expect("workspace");
...
fn a_broken_backend_recovers_without_resurrecting_dead_sessions() {
    ...
    let workspace = live
        .client
        .open_workspace(&std::env::temp_dir(), Some("chartr-recovery"))
        .expect("workspace");

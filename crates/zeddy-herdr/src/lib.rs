...
//! environment every herdr process zeddy launches is placed in, and the
//! [`control::Client`] carries one. An inherited `HERDR_SOCKET_PATH` from the
//! user's own shell therefore cannot split zeddy across two backends.
//!
...
pub const SUPPORTED_HERDR_VERSION: &str = "0.8.2-zeddy.5a2dee700eee";

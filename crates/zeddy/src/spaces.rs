//! The persisted list of folders zeddy calls spaces.
//!
//! This is a model below GPUI: folder picking belongs to the window, while
//! validation and persistence are testable without one. Chartr-zeddy is
//! deliberately isolated from older Chartr installations, so its registry
//! lives under the `chartr-zeddy` configuration namespace.
...
const HEADER: &str = "\
# chartr-zeddy's registered spaces, in sidebar order. Every folder here is one the
...
fn config_root_from(xdg: Option<OsString>, home: Option<PathBuf>) -> Result<PathBuf, Error> {
    ...
    home.filter(|home| !home.as_os_str().is_empty())
        .map(|home| home.join(".config/chartr-zeddy"))
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NotAFolder { path, source: Some(source) } => {
                write!(f, "{} is not a folder zeddy can register: {source}", path.display())
            }
...
mod tests {
    ...
    #[test]
    fn config_root_is_isolated_from_other_chartr_installations() {
        assert_eq!(
            config_root_from(Some("/xdg".into()), Some("/home/op".into())).unwrap(),
            PathBuf::from("/xdg/chartr-zeddy")
        );
        assert_eq!(
            config_root_from(None, Some("/home/op".into())).unwrap(),
            PathBuf::from("/home/op/.config/chartr-zeddy")
        );
    }

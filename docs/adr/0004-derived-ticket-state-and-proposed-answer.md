...
The split this ADR opens with **survives untouched**: ticket state is derived from `.plan/` markdown, session state is the harness's own, and the harness knows a session finished by watching an `## Answer` appear — never by asking the agent. What is withdrawn is everything the *gate* hung off that split.

- **`## Proposed Answer` is no longer a status.** `StatusProposed`, its derivation, and the promotion-at-approval write are gone with the review feature. The derived-status table is exactly wayfinder's four values again — open, claimed, resolved, out_of_scope — so the harness now extends the markdown adapter by *nothing*, rather than by one value.

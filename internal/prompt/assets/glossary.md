...
- **Frontier** — the open, unblocked tickets: the edge of the known. A blocker
  counts as cleared only when its answer is written *and* (on an implementation
  map) a human has approved it.
...
- **`## Answer`** — a resolved ticket's blessed conclusion. On disk, an Answer
  means the ticket is settled.
- **`## Proposed Answer`** — an implementation ticket's committed-but-ungated
  result: the work is done and written up, but no review has blessed it. It is

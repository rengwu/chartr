...
// The surface stands on its own with nothing registered: the agent library
// resolves with no space in play, so "what are my agents and where do they live"
// is answerable up front — and the open action is reachable without a space id.
//
// The open resolves a *named* layer server-side and refuses anything else, so a
// client-supplied path can never reach the editor (story 45); the names the
// retired committed-config and skill layers used to answer to are simply unknown
// now, and refused with everything else.
func TestGlobalLayersResolveWithoutASpace(t *testing.T) {
	...
	for _, bad := range []string{
		...
		if code, _ := h.Post("/api/config/open", map[string]string{"layer": bad}); code != 400 {
			t.Errorf("global open %q = %d, want 400 — only server-known names resolve", bad, code)
		}
	}
}

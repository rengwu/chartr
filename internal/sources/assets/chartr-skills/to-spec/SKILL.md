---
name: to-spec
description: Turn the current conversation into a spec, saved beside its map — no interview, just synthesis of what you've already discussed.
---
...
3. Write the spec using the template below, then save it as `spec.md`, in the directory of the map this work belongs to — reuse that directory if one already exists for this effort, or create it (using a short kebab-case name for the work) if this is the first document for it. A map's own directory is committed to version control, so a spec saved there is the project's shared planning memory, not a scratch file. Tell the user the path.

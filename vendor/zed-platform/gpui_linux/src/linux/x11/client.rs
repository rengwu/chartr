    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::FocusOut(event) => {
                ...
                let close_popup = window.is_anchored_popup();
                let mut state = self.0.borrow_mut();
...
impl LinuxClient for X11Client {
    ...
    fn open_window(
        ...
        let mut state = self.0.borrow_mut();
        let popup_parent = match &params.kind {
            WindowKind::AnchoredPopup(options) => Some(options.parent),
            _ => None,
        };
        let parent_window = if let Some(parent) = popup_parent {
            state
                .windows
                .values()
                .find(|window| window.window.state.borrow().handle == parent)

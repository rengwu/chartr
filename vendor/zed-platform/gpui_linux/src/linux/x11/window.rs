...
use gpui::{
    ...
    WindowDecorations, WindowKind, WindowParams, popup::PopupOptions, px,
};
...
fn place_popup(
    ...
    let gravity_offset = match gravity {
        G::Center => Point::new(popup_size.width / 2., popup_size.height / 2.),
        G::Top => Point::new(popup_size.width / 2., popup_size.height),
        G::Bottom => Point::new(popup_size.width / 2., px(0.)),
        G::Left => Point::new(popup_size.width, popup_size.height / 2.),
        G::Right => Point::new(px(0.), popup_size.height / 2.),
        G::TopLeft => Point::new(popup_size.width, popup_size.height),
        G::BottomLeft => Point::new(popup_size.width, px(0.)),
        G::TopRight => Point::new(px(0.), popup_size.height),
...
impl X11WindowState {
    pub fn new(
        ...
        let logical_bounds = if let WindowKind::AnchoredPopup(options) = &params.kind {
            let parent = parent_window
                .as_ref()
                .ok_or_else(|| anyhow!("anchored popup parent window not found"))?;
            let parent = parent.state.borrow();
            anchored_popup_bounds(
                options,
                params.bounds.size,
                parent.bounds,
                parent.display.visible_bounds(),
            )
        } else {
            params.bounds
        };
        let mut bounds = logical_bounds.to_device_pixels(scale_factor);
        ...
        let setup_result = maybe!({
            ...
            if matches!(
                &params.kind,
                WindowKind::Floating | WindowKind::Dialog | WindowKind::AnchoredPopup(_)
            ) {
            ...
            Ok(Self {
                ...
                anchored_popup: matches!(&params.kind, WindowKind::AnchoredPopup(_)),
...
impl X11WindowStatePtr {
    ...
    pub fn is_anchored_popup(&self) -> bool {
        self.state.borrow().anchored_popup
    }
...
impl PlatformWindow for X11Window {
    ...
    fn activate(&self) {
        if self.0.is_anchored_popup() {
            check_reply(
                || "X11 SetInputFocus for anchored popup failed.",
                self.0.xcb.set_input_focus(

...
impl Platform for MacPlatform {
    ...
    fn open_window(
        ...
        let popup_parent = match &options.kind {
            WindowKind::AnchoredPopup(popup) => Some(
                MacWindow::native_window_for_handle(popup.parent)
                    .context("anchored popup parent window not found")?,
            ),
            _ => None,
        };

...
use gpui::{
    ...
    WindowParams, point, popup::PopupOptions, px, size,
};
#[cfg(any(test, feature = "test-support"))]
use image::RgbaImage;
...
struct MacWindowState {
    ...
    // The owner of an anchored popup. AppKit child windows stay above their
    // parent without taking the application out of its active state.
    popup_parent: Option<id>,
    close_on_resign_key: bool,
}
...
fn anchored_popup_bounds(
    options: &PopupOptions,
    popup_size: Size<Pixels>,
    parent_bounds: Bounds<Pixels>,
    visible_bounds: Bounds<Pixels>,
) -> Bounds<Pixels> {
    use gpui::popup::PopupConstraintAdjustment as Adjustment;

    let anchor_rect = Bounds::new(
        parent_bounds.origin + options.anchor_rect.origin,
        options.anchor_rect.size,
    );
    let mut anchor = options.anchor;
    let mut gravity = options.gravity;
    let mut bounds = place_popup(
        anchor_rect,
        popup_size,
        anchor,
        gravity,
    ...
    if options
        ...
        .contains(Adjustment::FLIP_X)
        && horizontal_overflow(bounds, visible_bounds) > 0.0
    {
        let candidate = place_popup(
            anchor_rect,
            popup_size,
        ...
        if horizontal_overflow(candidate, visible_bounds)
            < horizontal_overflow(bounds, visible_bounds)
        {
            anchor = flip_anchor_x(anchor);
            gravity = flip_gravity_x(gravity);
            bounds = candidate;
        }
    }
    ...
    if options
        .constraint_adjustment
        .contains(Adjustment::FLIP_Y)
        && vertical_overflow(bounds, visible_bounds) > 0.0
    {
        let candidate = place_popup(
            anchor_rect,
            popup_size,
        ...
        if vertical_overflow(candidate, visible_bounds)
            < vertical_overflow(bounds, visible_bounds)
        {
            bounds = candidate;
        }
    }
    ...
    if options
        .constraint_adjustment
        .contains(Adjustment::SLIDE_X)
    {
        bounds.origin.x = clamp_axis(
            bounds.origin.x,
            visible_bounds.left(),
            visible_bounds.right() - bounds.size.width,
        );
    }
    if options
        .constraint_adjustment
        .contains(Adjustment::SLIDE_Y)
    {
        bounds.origin.y = clamp_axis(
            bounds.origin.y,
            visible_bounds.top(),
            visible_bounds.bottom() - bounds.size.height,
        );
    }
    bounds
}
...
fn place_popup(
    ...
    popup_size: Size<Pixels>,
    anchor: gpui::popup::PopupAnchor,
    gravity: gpui::popup::PopupGravity,
    offset: Point<Pixels>,
) -> Bounds<Pixels> {
    use gpui::popup::{PopupAnchor as A, PopupGravity as G};

    let anchor_point = match anchor {
        A::Center => anchor_rect.center(),
        A::Top => anchor_rect.top_center(),
        A::Bottom => anchor_rect.bottom_center(),
        A::Left => anchor_rect.left_center(),
        A::Right => anchor_rect.right_center(),
        A::TopLeft => anchor_rect.origin,
        A::BottomLeft => anchor_rect.bottom_left(),
        A::TopRight => anchor_rect.top_right(),
        A::BottomRight => anchor_rect.bottom_right(),
    };
    let gravity_offset = match gravity {
        G::Center => point(popup_size.width / 2., popup_size.height / 2.),
        G::Top => point(popup_size.width / 2., popup_size.height),
        G::Bottom => point(popup_size.width / 2., px(0.)),
        G::Left => point(popup_size.width, popup_size.height / 2.),
        G::Right => point(px(0.), popup_size.height / 2.),
        G::TopLeft => point(popup_size.width, popup_size.height),
        G::BottomLeft => point(popup_size.width, px(0.)),
        G::TopRight => point(px(0.), popup_size.height),
        G::BottomRight => Point::default(),
    };
    Bounds::new(anchor_point - gravity_offset + offset, popup_size)
}

fn horizontal_overflow(bounds: Bounds<Pixels>, visible: Bounds<Pixels>) -> f32 {
    (visible.left() - bounds.left()).as_f32().max(0.)
        + (bounds.right() - visible.right()).as_f32().max(0.)
}

fn vertical_overflow(bounds: Bounds<Pixels>, visible: Bounds<Pixels>) -> f32 {
    (visible.top() - bounds.top()).as_f32().max(0.)
        + (bounds.bottom() - visible.bottom()).as_f32().max(0.)
}

fn clamp_axis(value: Pixels, minimum: Pixels, maximum: Pixels) -> Pixels {
    px(value
        .as_f32()
        .clamp(minimum.as_f32(), maximum.max(minimum).as_f32()))
}

fn flip_anchor_x(anchor: gpui::popup::PopupAnchor) -> gpui::popup::PopupAnchor {
...
fn flip_anchor_y(anchor: gpui::popup::PopupAnchor) -> gpui::popup::PopupAnchor {
    use gpui::popup::PopupAnchor::*;
    match anchor {
        Top => Bottom,
        Bottom => Top,
...
fn flip_gravity_x(gravity: gpui::popup::PopupGravity) -> gpui::popup::PopupGravity {
...
fn flip_gravity_y(gravity: gpui::popup::PopupGravity) -> gpui::popup::PopupGravity {
    use gpui::popup::PopupGravity::*;
    match gravity {
        Top => Bottom,
        Bottom => Top,
...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            if let (WindowKind::AnchoredPopup(options), Some(parent)) = (&kind, popup_parent) {
                let parent_bounds = get_window_state(&*parent).lock().bounds();
                bounds = anchored_popup_bounds(
                    options,
                    bounds.size,
                    parent_bounds,
            ...
            if let Some(parent) = popup_parent {
            ...
            NSWindow::setFrameTopLeftPoint_(native_window, window_rect.origin);
            {
                ...
                window_state.popup_parent = popup_parent;
            }
    ...
    pub fn active_window() -> Option<AnyWindowHandle> {
        unsafe {
            let app = NSApplication::sharedApplication(nil);
            // The main window can remain the workspace while a Settings
            // window or a popup owns keyboard focus. Report the key window so
            // deferred UI work cannot mistake that workspace for the active one.
            let key_window: id = msg_send![app, keyWindow];
    ...
    pub(crate) fn native_window_for_handle(handle: AnyWindowHandle) -> Option<id> {
        unsafe {
            let app = NSApplication::sharedApplication(nil);
            ...
            for index in 0..count {
                ...
                if is_gpui_window(window) && get_window_state(&*window).lock().handle == handle {
                    return Some(window);
                }
            }
            None
...
impl Drop for MacWindow {
    fn drop(&mut self) {
        ...
        let popup_parent = this.popup_parent.take();
...
extern "C" fn window_did_change_key_status(this: &Object, selector: Sel, _: id) {
    ...
    let executor = lock.foreground_executor.clone();
    let close_on_resign_key = lock.close_on_resign_key;
    let native_window = lock.native_window;
    let closed = lock.closed.clone();
    drop(lock);

    if selector == sel!(windowDidResignKey:) && close_on_resign_key {
        executor
            .spawn(async move {
                if_window_not_closed(closed, || unsafe {
                    let _: () = msg_send![native_window, close];
                });
            })
            .detach();
        return;

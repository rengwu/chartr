...
/// Controls where a standalone terminal places the fractional row of spare
/// vertical space left after fitting an integral terminal grid into its view.
#[derive(Default, Clone, Copy, Debug, PartialEq, Eq)]
pub enum TerminalVerticalAlignment {
    /// Preserve Zed's default: full and alternate-screen terminals put the
    /// spare pixels above the grid so its bottom edge remains fixed.
    #[default]
    BottomWhenFull,
    /// Put spare pixels below the grid, keeping row zero at a stable origin.
    Top,
}
...
impl TerminalView {
    ...
    /// Set the standalone grid's vertical alignment policy.
    pub fn set_vertical_alignment(
        &mut self,
        alignment: TerminalVerticalAlignment,
        cx: &mut Context<Self>,
    ) {

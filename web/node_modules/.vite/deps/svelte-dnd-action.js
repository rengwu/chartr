...
function overrideItemIdKeyNameBeforeInitialisingDndZones(newKeyName) {
	if (activeDndZoneCount > 0) throw new Error("can only override the id key before initialising any dndzone");
	if (typeof newKeyName !== "string") throw new Error("item id key has to be a string");
	printDebug(function() {
...
function setDebugMode(isDebug) {
	if (isDebug) printDebug = function printDebug(generateMessage) {
		var logFunction = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : console.debug;
		var message = generateMessage();
		if (Array.isArray(message)) logFunction.apply(void 0, _toConsumableArray(message));
		else logFunction(message);
	};
	else printDebug = function printDebug() {};
}
/**
...
function handleDraggedEntered(e) {
	...
	if (_dzToConfig$get.dropFromOthersDisabled && e.currentTarget !== originDropZone) {
		printDebug(function() {
...
function handleDraggedLeft(e) {
	...
	if (_dzToConfig$get2.dropFromOthersDisabled && e.currentTarget !== originDropZone && e.currentTarget !== shadowElDropZone) {
		printDebug(function() {
...
function handleDraggedIsOverIndex(e) {
	printDebug(function() {
		return [
			"dragged is over index",
			e.currentTarget,
			e.detail
		];
	});
	var _dzToConfig$get3 = dzToConfig$1.get(e.currentTarget), originalItems = _dzToConfig$get3.items;
	if (_dzToConfig$get3.dropFromOthersDisabled && e.currentTarget !== originDropZone) {
		printDebug(function() {
			return "drop is currently disabled";
		});
		return;
...
function dndzone$2(node, options) {
	...
	printDebug(function() {
		return ["dndzone good to go options: ".concat(toString(options), ", config: ").concat(toString(config)), { node }];
	});
...
var DEFAULT_ARIA_STRINGS = {
	dragStarted: function dragStarted(_ref) {
		var itemLabel = _ref.itemLabel, zoneLabel = _ref.zoneLabel, canMoveBetweenZones = _ref.canMoveBetweenZones;
	...
	movedToPosition: function movedToPosition(_ref2) {
		var itemLabel = _ref2.itemLabel, zoneLabel = _ref2.zoneLabel, position = _ref2.position;
	...
	movedToZoneEnd: function movedToZoneEnd(_ref3) {
		var itemLabel = _ref3.itemLabel, zoneLabel = _ref3.zoneLabel;
	...
	movedToZoneStart: function movedToZoneStart(_ref4) {
		var itemLabel = _ref4.itemLabel, zoneLabel = _ref4.zoneLabel;
...
var instructionCtx = { keyboardDragTrigger: DEFAULT_KEYBOARD_DRAG_TRIGGER };
...
function initAriaOnBrowser() {
	...
	Object.entries(INSTRUCTION_ID_TO_STRING_KEY).forEach(function(_ref7) {
		var _ref8 = _slicedToArray(_ref7, 2), id = _ref8[0], key = _ref8[1];
...
function setAriaStrings(overrides) {
	if (overrides === null || overrides === void 0) ariaStrings = _objectSpread2({}, DEFAULT_ARIA_STRINGS);
	else {
		if (_typeof(overrides) !== "object" || Array.isArray(overrides)) throw new Error("setAriaStrings expects an object or null but instead got a ".concat(_typeof(overrides), ", ").concat(toString(overrides)));
		Object.keys(overrides).forEach(function(key) {
			if (!Object.prototype.hasOwnProperty.call(DEFAULT_ARIA_STRINGS, key)) throw new Error("Can't set non existing aria string ".concat(key, "! Supported strings: ").concat(Object.keys(DEFAULT_ARIA_STRINGS)));
			var value = overrides[key];
			if (FUNCTION_ARIA_STRING_KEYS.includes(key) && typeof value !== "function") throw new Error("".concat(key, " should be a function but instead it is a ").concat(_typeof(value), ", ").concat(toString(value)));
			if (STRING_OR_FUNCTION_ARIA_STRING_KEYS.includes(key) && typeof value !== "string" && typeof value !== "function") throw new Error("".concat(key, " should be a string or a function but instead it is a ").concat(_typeof(value), ", ").concat(toString(value)));
			if (STRING_ONLY_ARIA_STRING_KEYS.includes(key) && typeof value !== "string") throw new Error("".concat(key, " should be a string but instead it is a ").concat(_typeof(value), ", ").concat(toString(value)));
		});
...
function refreshInstructions() {
	...
	Object.entries(INSTRUCTION_ID_TO_STRING_KEY).forEach(function(_ref9) {
		var _ref10 = _slicedToArray(_ref9, 2), id = _ref10[0], key = _ref10[1];
...
function setKeyboardDragTrigger(trigger) {
	if (trigger === null || trigger === void 0) activeTrigger = DEFAULT_KEYBOARD_DRAG_TRIGGER;
	else if (typeof trigger !== "string" || !Object.prototype.hasOwnProperty.call(KEYBOARD_DRAG_TRIGGER_KEYS, trigger)) throw new Error("setKeyboardDragTrigger expects one of ".concat(Object.keys(KEYBOARD_DRAG_TRIGGER_KEYS).map(function(key) {
		return "\"".concat(key, "\"");
	}).join(", "), " or null but instead got a ").concat(_typeof(trigger), ", ").concat(toString(trigger)));
	else activeTrigger = trigger;
...
function handleZoneFocus(e) {
	...
	if (newlyFocusedDz.getBoundingClientRect().top < focusedDz.getBoundingClientRect().top || newlyFocusedDz.getBoundingClientRect().left < focusedDz.getBoundingClientRect().left) {
		...
		if (!autoAriaDisabled) announceToScreenReader("movedToZoneEnd", {
		...
		if (!autoAriaDisabled) announceToScreenReader("movedToZoneStart", {
			itemLabel: focusedItemLabel,
			zoneLabel: focusedDzLabel,
			position: 1,
			count: targetItems.length
...
function dndzone$1(node, options) {
	...
	function handleKeyDown(e) {
		...
		switch (e.key) {
			case "Enter":
			case " ":
				if (!isKeyboardDragTriggerKey(e.key)) return;
			...
			case "ArrowRight":
				...
				if (idx < children.length - 1) {
					if (!config.autoAriaDisabled) announceToScreenReader("movedToPosition", {
						itemLabel: focusedItemLabel,
						zoneLabel: focusedDzLabel,
						position: idx + 2,
						count: items.length
	...
	function configure(_ref2) {
		...
		if (itemMovedToThisZone) refreshActiveDragTabIndices();
	}
	configure(options);
	var handles = {
		update: function update(newOptions) {

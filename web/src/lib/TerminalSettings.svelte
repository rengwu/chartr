...
<section class="flex flex-col gap-2">
  ...
  <p class="text-xs leading-relaxed text-muted-foreground">
    ...
    <code class="font-mono">terminal.toml</code>. Edit it in your own editor and every open terminal
    re-applies it. Values it sets are shown in full; everything else is the default in force. A
    value it gets wrong is ignored with a warning on each space's settings.
  </p>
            ...
            {#each g.rows as r (r.label)}
              ...
              <dd class="flex min-w-0 items-center gap-1.5">
                ...
                <span
                  ...
                  title={r.set ? `set in terminal.toml: ${r.value}` : `default: ${r.value}`}
                >
                  {r.value}
                </span>
              </dd>

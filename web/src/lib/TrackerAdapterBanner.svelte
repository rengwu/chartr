<script lang="ts">
  ...
  const foreignHint = $derived(
    offer.remoteHint ? hintLabel[offer.remoteHint] : undefined,
  );
...
<!-- A quiet banner, emphasised (border-ring) while chartr wants a write and neutral
     ...
     every action a Button primitive. Placement (margins) is the parent's — this is
     a pure card, dropped under the space header for stale/foreign and inside the
     maps-pane picker for the absent offer. -->
<div
  class="flex flex-col gap-1.5 rounded-md border p-2.5 {offer.state ===
  'foreign'
    ? 'border-border'
    : 'border-ring'}"
>
  <div class="flex items-start gap-2.5">
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-0.5">
      {#if offer.state === "absent"}
        <p class="text-xs leading-relaxed">
          Make the wayfinder skill save its maps where chartr can read them.
        </p>
      {:else if offer.state === "stale"}
        <p class="text-xs leading-relaxed">
          chartr's tracker adapter has an update.
        </p>
      {:else}
        <p class="text-xs leading-relaxed">
          An existing tracker config is here{foreignHint
            ? ` — looks like ${foreignHint}`
            : ""}.
        </p>
      {/if}
      <code
    ...
    <div class="flex shrink-0 items-center gap-1.5">
      {#if offer.state === "foreign"}
        ...
        <Button variant="secondary" size="xs" disabled={busy} onclick={dismiss}
          >Leave</Button
        >
        <Button
          variant="destructive"
      ...
      {:else}
        <Button variant="default" size="xs" disabled={busy} onclick={install}>
          {offer.state === "stale" ? "Refresh" : "Install adapter"}
        </Button>
        <Button
          variant="ghost"
          size="icon-xs"
...
<Modal
  open={confirming}
  title="Replace the existing tracker config?"
